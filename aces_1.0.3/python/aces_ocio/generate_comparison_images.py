...
def generate_comparison_images(aces_ctl_directory,
    ...
    """
    Generates a set of images from CTL and from OCIO for all Output Transforms.
    ...
    use_ocio = config_directory != None
    ...
    for odt_ctl_name, odt_values in odt_info.iteritems():
        ...
        if 'transformCTL' in odt_values:
            ...
            if use_ocio:
              # OCIO render
              output_transform_image_ocio = os.path.join(destination_directory,
              ...
              # Difference image
              output_transform_image_diff = os.path.join(destination_directory,
            ...
            1. The CTL inverse transforms applied to the forwarded transformed image
            2. The OCIO inverse transforms applied to the forwarded transformed image
            Three difference images per output transform
            1. The difference between the the OCIO and CTL results
            2. The difference between the CTL result and the original image
            ...
            if 'transformCTLInverse' in odt_values:
                ...
                inverse_output_transform_image_ctl = os.path.join(destination_directory,
                    ".".join([inverse_output_transform_image, 'ctl', image_format]) )
                ...
                if use_ocio:
                  # OCIO render
                  inverse_output_transform_image_ocio = os.path.join(destination_directory,
                      ".".join([inverse_output_transform_image, 'ocio', image_format]) )
                  ...
                  inverse_output_transform_image_diff1 = os.path.join(destination_directory,
                      ".".join([inverse_output_transform_image, 'diff_ocio_ctl', image_format]) )
                  ...
                  inverse_output_transform_image_diff3 = os.path.join(destination_directory,
                      ".".join([inverse_output_transform_image, 'diff_ocio_original', image_format]) )
                ...
                inverse_output_transform_image_diff2 = os.path.join(destination_directory,
                    ".".join([inverse_output_transform_image, 'diff_ctl_original', image_format]) )
...
def main():
    ...
    usage = '%prog [options]\n'
    usage += '\n'
    usage += 'A script to compare results between OCIO and CTL for an ACES release.\n'
    usage += '\n'
